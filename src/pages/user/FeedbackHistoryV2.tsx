...
/**
 * FeedbackHistoryV2 — the new-design ("HomeV2" system) variant of Feedback
 * History. Flag-gated (new_user_surfaces) additive swap; the classic page is
 * unchanged and remains at /feedback/classic. Same data/hooks/logic — only the
 * presentation is re-skinned to the cream panel + tokens + serif heading.
 * RTL-safe (logical CSS).
 */
export default function FeedbackHistoryV2() {
    ...
    <UserLayout>
      <V2Panel>
        <div className="flex flex-col gap-1 text-start">
          <h1 className="text-xl font-bold font-serif text-ink">{t("coaching:feedback.history")}</h1>
          <p className="text-[13px] text-body-slate">{t("coaching:feedback.reviewDescription")}</p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {statsLoading ? (
            Array.from({ length: 4 }).map((_, i) => (
              <V2Card key={i} className="p-3.5 animate-pulse">
            ...
            [
              ...
              <V2Card key={s.label} className="p-3.5">
                <div className="text-xs text-body-slate">{s.label}</div>
        ...
        <DataCard title={t("coaching:feedback.feedback")}>
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <select
              ...
              className="border border-hairline rounded-lg px-3 py-1.5 text-[13px] outline-none focus:border-ink"
            >
          ...
          {listLoading && (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ms-2 text-sm text-body-slate">{t("coaching:feedback.loadingFeedback")}</span>
            </div>
          )}
            ...
            <>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-[#f9fafb] border-b border-hairline">
                      {["Date", "Coach", "Rating", "Sentiment", "Correction"].map((h) => (
                        <th key={h} className="text-start text-[10px] font-bold uppercase tracking-wider text-body-slate px-3 py-2.5">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {feedback.map((f: FeedbackEntry) => (
                      <tr key={f.id} className="border-b border-hairline hover:bg-[#f9fafb]">
                        <td className="px-3 py-2.5 text-xs text-body-slate whitespace-nowrap">{formatDate(f.created_at)}</td>
                        <td className="px-3 py-2.5 text-[13px] font-medium text-body-slate">{f.coach_id}</td>
                        <td className="px-3 py-2.5"><StarRating rating={f.rating ?? 0} /></td>
                        <td className="px-3 py-2.5"><RatingBadge rating={f.rating ?? 0} /></td>
                        <td className="px-3 py-2.5 text-[13px] text-body-slate max-w-[300px] truncate">
                          {f.correction_text || <span className="text-mute">—</span>}
                        </td>
                      </tr>
                    ))}
                    {feedback.length === 0 && (
                      <tr><td colSpan={5} className="px-3 py-8 text-center text-[13px] text-mute">No feedback found</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
                ...
                <div className="flex items-center justify-between mt-4 pt-3 border-t border-hairline">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 text-xs text-body-slate px-2.5 py-1.5 rounded-lg border border-hairline hover:bg-[#f9fafb] disabled:opacity-40 disabled:cursor-not-allowed"
                    >
