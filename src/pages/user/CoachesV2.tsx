...
/**
 * CoachesV2 — the new-design ("HomeV2" system) variant of Coaches. Flag-gated
 * (new_user_surfaces) additive swap; the classic page is unchanged and remains
 * at /coaches/classic. Same data/hooks/logic — only the presentation is
 * re-skinned to the cream panel + tokens + serif heading. RTL-safe (logical CSS).
 */
export default function CoachesV2() {
      ...
      <V2Panel>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex flex-col text-start gap-1">
            <h1 className="font-serif text-[22px] leading-tight tracking-tight text-ink">{t("dashboard:manageCoaches")}</h1>
            <p className="text-xs text-body-slate">{t("dashboard:coachesConfigDescription")}</p>
          </div>
          <div className="w-full sm:max-w-xs">
            <IconInput
              ...
              className="rounded-xl bg-white border border-hairline"
            />
          </div>
        </div>
