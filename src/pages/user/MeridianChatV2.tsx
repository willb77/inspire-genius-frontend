...
export default function MeridianChatV2() {
  ...
  // V2 — the Export control lives in the sticky header (next to History); the
  // page owns the modal state since the stacked ChatWindow no longer renders
  // its own header/export trigger.
    ...
    <UserLayout>
      ...
      >
        ...
        <div className="flex items-center gap-2" data-tour="meridian-dropdowns">
          ...
          />
          {/* Export — moved next to History per the V2 layout. */}
          <button
            type="button"
      ...
      {/* V2 canvas — nav (UserLayout) │ tile rail │ [Compose / Conversation].
          Below 1024px the rail stacks above the chat column. The grid gets a
          fixed viewport-derived height so the stacked ChatWindow's Conversation
          card can flex-grow and scroll internally. */}
      <div
        className="grid h-[calc(100vh-8rem)] grid-cols-1 gap-5 lg:grid-cols-[320px_minmax(0,1fr)]"
      ...
      >
        ...
        <div className="flex min-h-0 min-w-0 flex-col" data-tour="chat-window">
          {reviewConversationIds.length > 0 && (
            <div className="mb-2 rounded-lg border border-hairline bg-panel px-3 py-2 text-xs text-mute">
      ...
      {/* Export chat modal — opened from the header Export button (next to
