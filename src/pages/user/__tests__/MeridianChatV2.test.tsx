...
import MeridianChatV2 from "../MeridianChatV2";
...
describe("MeridianChatV2", () => {
  ...
  it("renders ChatWindow in stacked (V2 two-card) layout", () => {
    renderPage();
    expect(capturedChatWindowProps.stacked).toBe(true);
  });

  it("renders the Meridian tile rail (Active/History/Last5/Projects/Knowledge)", () => {
    renderPage();
    expect(screen.getByTestId("meridian-tile-rail")).toBeInTheDocument();
    expect(screen.getByTestId("rail-toggle-active")).toBeInTheDocument();
    expect(screen.getByTestId("rail-toggle-history")).toBeInTheDocument();
    expect(screen.getByTestId("rail-toggle-last5")).toBeInTheDocument();
    expect(screen.getByTestId("rail-toggle-projects")).toBeInTheDocument();
    expect(screen.getByTestId("rail-toggle-knowledge")).toBeInTheDocument();
  });

  it("renders the Export button in the header (next to History)", () => {
    renderPage();
