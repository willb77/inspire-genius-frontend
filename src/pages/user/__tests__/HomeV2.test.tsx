...
describe("HomeV2 — 2026-08-03 changes", () => {
  ...
  it("no longer carries the guide pill — it lives in UserTopHeader now", () => {
    // Moved 2026-08-25 (request) to sit beside "Welcome! {name}" in the app
    // header. Asserted here so the pill cannot quietly come back and render
    // twice on Home. Its own contract is covered by
    // components/shared/__tests__/UserTopHeader.test.tsx.
    wrap();
    expect(
      screen.queryByTestId("homev2-workspace-guide-video"),
    ).not.toBeInTheDocument();
    expect(screen.queryByTestId("workspace-guide-video")).not.toBeInTheDocument();
  });
});
