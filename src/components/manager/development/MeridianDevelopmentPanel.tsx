  ...
  return (
    <div className="flex h-full flex-col rounded-xl border border-slate-200 bg-white">
      <div className="flex items-center gap-2 border-b border-slate-100 p-3">
        <span className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-[#3B5BFF] to-[#2DD4BF] text-white">
      ...
      <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto p-3">
        {turns.length === 0 ? (
          <div className="space-y-2">
            <p className="text-xs text-slate-400">Suggested prompts</p>
            {SUGGESTED_PROMPTS[tab].map((p) => (
              <button
                key={p}
                type="button"
                onClick={() => send(p)}
                className="block w-full rounded-lg border border-slate-200 p-2 text-left text-xs text-slate-600 hover:border-[#3B5BFF] hover:text-slate-900"
              >
                {p}
              </button>
            ))}
          ...
          turns.map((turn) => (
            <div key={turn.id} className={cn("flex gap-2", turn.role === "user" ? "flex-row-reverse" : "")}>
              <span className={cn("flex h-6 w-6 shrink-0 items-center justify-center rounded-full", turn.role === "user" ? "bg-slate-200" : "bg-[#3B5BFF]/10")}>
                {turn.role === "user" ? <User className="h-3.5 w-3.5 text-slate-500" /> : <Bot className="h-3.5 w-3.5 text-[#3B5BFF]" />}
              </span>
              <div className={cn("max-w-[80%] whitespace-pre-wrap rounded-lg p-2 text-xs", turn.role === "user" ? "bg-slate-100 text-slate-700" : "bg-slate-50 text-slate-700")}>
                {turn.content}
              </div>
            </div>
          ))
        ...
        {isProcessing && currentResponse ? (
          <div className="flex gap-2">
            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#3B5BFF]/10">
              <Bot className="h-3.5 w-3.5 text-[#3B5BFF]" />
            </span>
            <div className="max-w-[80%] whitespace-pre-wrap rounded-lg bg-slate-50 p-2 text-xs text-slate-500">
      ...
      {staged ? (
        <Card className="m-3 border-[#3B5BFF]/40 bg-[#3B5BFF]/5">
          <CardContent className="space-y-2 p-3">
            <div className="text-xs font-semibold text-slate-700">Proposed action</div>
            <p className="text-xs text-slate-600">{staged.label}</p>
            <div className="flex gap-2">
      ...
      >
        <input
          ...
          className="flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-[#3B5BFF] focus:outline-none"
        />
